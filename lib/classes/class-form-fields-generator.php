...
// Verify the token on the server.
		...
		public function field_text( $option ) {
			...
			$received_person = get_post_meta( get_the_ID(), 'received_by', true );
			
			if ( 'user' == $default ) {
				
				$value = get_current_user_id() ? new WP_User( get_current_user_id() ) : wp_get_current_user();
				
				if ( ! empty( $value->first_name ) ) {
					if ( ! empty( $value->last_name ) ) {
						$value = ucfirst( $value->first_name . ' ' . $value->last_name );
					}
				}
				
				if ( ! empty ( $received_person ) ) {
					$value = ucfirst( $received_person );
				}
			...
			//check readonly
			if ( ! empty( $readonly ) && 'yes' == $readonly ) {
				$readonly = "readonly";
			} else {
				$readonly = "";
			}
			...
			if ( ! is_super_admin( get_current_user_id() ) && isset( $_GET['action'] ) ) {
				if ( $_GET['action'] == 'edit' ) {
					$readonly = "readonly";
				}
			}
         ...
         field-text-wrapper-<?php echo $id; ?>">
                ...
                <input type='text' name='<?php echo esc_attr( $field_name ); ?>'
                       id='<?php echo esc_attr( $field_id ); ?>' <?php echo $readonly; ?>
                       placeholder='<?php echo esc_attr( $placeholder ); ?>' value='<?php echo esc_attr( $value ); ?>'/><span
                        class="<?php echo esc_attr( $field_id ); ?>" style="display: none">please wait</span>
                <div class="error-mgs"></div>
            </div>
...
"><?php echo( $remove_text ); ?></span>
		...
		public function field_textarea( $option ) {
            ...
            <div <?php if ( ! empty( $depends ) ) { ?> data-depends="[<?php echo $depends; ?>]" <?php } ?>
                ...
                <textarea name='<?php echo esc_attr( $field_name ); ?>' id='<?php echo esc_attr( $field_id ); ?>'
                          cols='40' rows='5'
                          placeholder='<?php echo $placeholder; ?>' <?php echo $readonly; ?>><?php echo esc_attr( $value ); ?></textarea>
                <div class="error-mgs"></div>
            </div>
		...
		public function field_select( $option ) {
			...
			$limit    = ! empty( $option['limit'] ) ? $option['limit'] : '';
			$roles    = ! empty( $option['user_roles'] ) ? $option['user_roles'] : '';
			$value    = isset( $option['value'] ) ? $option['value'] : '';
			$value    = ! empty( $value ) ? $value : $default;
		...
		public function field_user( $option ) {
            ...
            field-user-multi-wrapper-<?php echo $id; ?>">
                ...
                <div class="user-list">
                    <div class="ppof-button select-user"><?php echo __( 'Choose User',
							'pickplugins-options-framework' ); ?></div>
                    <div class="search-user"><input class="" type="text"
                                                    placeholder="<?php echo __( 'Start typing...',
		...
		public function field_icon( $option ) {
            ...
            field-icon-wrapper-<?php echo $id; ?>">
                ...
                <div class="icon-list">
                    <div class="ppof-button select-icon"><?php echo __( 'Choose Icon',
							'pickplugins-options-framework' ); ?></div>
            ...
            <div <?php if ( ! empty( $depends ) ) { ?> data-depends="[<?php echo $depends; ?>]" <?php } ?>
             ...
             field-number-wrapper-<?php echo $id; ?>">
                <input type='number' class='' name='<?php echo $field_name; ?>' id='<?php echo $field_id; ?>'
                       placeholder='<?php echo $placeholder; ?>' <?php echo $readonly; ?>
                       value='<?php echo $value; ?>' min="<?php echo $min; ?>"/>
                <div class="error-mgs"></div>
            </div>
		...
		public function field_grid( $option ) {
			...
			$args       = isset( $option['args'] ) ? $option['args'] : "";
			$conditions = isset( $option['conditions'] ) ? $option['conditions'] : array();
			$widths     = isset( $option['width'] ) ? $option['width'] : array(
				'768px'  => '100%',
				'992px'  => '50%',
				'1200px' => '30%',
			);
			$heights    = isset( $option['height'] ) ? $option['height'] : array(
				'768px'  => 'auto',
				'992px'  => '250px',
				'1200px' => '250px',
			);
            ...
            field-image-link-wrapper-<?php echo $id; ?>">
                ...
                <div class="val-wrap">
                    ...
                    <span class='ppof-button upload'
                          id='media_upload_<?php echo $id; ?>'><?php echo __( 'Upload',
							'pickplugins-options-framework' ); ?></span>
		...
		public function field_media( $option ) {
            ...
            field-media-wrapper-<?php echo $id; ?>">
                ...
                <div class='ppof-button clear'
                     id='media_clear_<?php echo $id; ?>'><?php echo __( 'Clear',
						'pickplugins-options-framework' ); ?></div>
                <div class="error-mgs"></div>
            </div>
		...
		public function get_tax_data( $args ) {
			foreach ( $this->get_rep_taxonomies_array( $args ) as $argIndex => $argName ):
				$selected = ( $argIndex == $value ) ? 'selected' : ''; ?>
                <option <?php echo $selected; ?>
                        value="<?php echo $argIndex; ?>"><?php echo $argName; ?></option> <?php endforeach;
		}
...
//			}
			...
			if ( strpos( $string, 'USER_IDS_ARRAY' ) !== false ) {
				return $this->get_user_ids_array( $string );
			}
...
//			if ( $taxonomy == "status" ) {
//				array_push($taxonomies,"Received");
//			}
			
			foreach ( $terms as $term ) {
				
				if ( $taxonomy == 'priority' ) {
					$id = get_term_meta( $term->term_id, 'priority_value', true );
				} else {
					$id = $term->term_id;
				}
		...
		public function get_user_ids_array( $string ) {
			
			preg_match_all( "/\%([^\]]*)\%/", $string, $matches );
			$role_name = $matches[1][0];
			...
			foreach ( $users as $user ) {
				...
				$user_ids[ $user->ID ] = $user->first_name . " " . $user->last_name . '(#' . $user->ID . ')';
	...
	global $csmscore;
	$csmscore = new FormFieldsGenerator();
}